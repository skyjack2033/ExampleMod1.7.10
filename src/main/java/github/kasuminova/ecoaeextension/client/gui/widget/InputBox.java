...
// removed static import of DEFAULT_FONT_HEIGHT
...
public class InputBox extends DynamicWidget {
    ...
    public void render(final WidgetGui gui, final RenderSize renderSize, final RenderPos renderPos, final MousePos mousePos) {
        ...
        int fontHeight = 9; // DEFAULT_FONT_HEIGHT equivalent
        int heightOffset = height > fontHeight ? (height - fontHeight) / 2 : 0;
    ...
    public boolean onKeyTyped(final char typedChar, final int keyCode) {
        ...
        switch (inputType) {
            case STRING: field.textboxKeyTyped(typedChar, keyCode); switchResult = true; break;
            case NUMBER: {
    ...
    protected boolean processNumberTypeInput(final char typedChar, final int keyCode) {
        ...
        // 判断进制并检查字符有效性
        boolean isValid;
        switch (getActiveBase(newInput)) {
            case 2: isValid = typedChar == '0' || typedChar == '1'; break;
            case 8: isValid = Character.toString(typedChar).matches("[0-7]"); break;
            case 10: isValid = Character.isDigit(typedChar) || typedChar == '.'; break;
            case 16: isValid = Character.toString(typedChar).matches("[0-9a-fA-F]"); break;
            // 如果无法识别进制，默认无效
